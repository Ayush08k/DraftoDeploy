...
const AuroraBackground: React.FC<AuroraBackgroundProps> = ({
  ...
  return (
    <div
      ...
      className={`relative flex flex-col w-full min-h-screen items-center justify-center bg-black text-slate-50 overflow-hidden ${className}`}
    >
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none" aria-hidden="true">
        ...
        {Array.from({ length: starCount }).map((_, i) => {
          ...
          return (
            <motion.div
              key={i}
              className="absolute w-1 h-1 bg-white/70 rounded-full pointer-events-none"
              style={{ top, left }}
              initial={{ opacity: 0 }}
              animate={{
                opacity: [0, 0.7, 0],
